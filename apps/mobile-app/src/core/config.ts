...
export const getApiUrl = (): string => {
  const url = Config.EXPO_PUBLIC_API_URL;
  if (url) return url;
  if (__DEV__) {
...
export const getTelemetryApiKey = (): string | undefined =>
  Config.EXPO_PUBLIC_TELEMETRY_API_KEY || undefined;
...
export const getTelemetryBundleId = (): string =>
  Config.EXPO_PUBLIC_TELEMETRY_BUNDLE_ID ?? 'mx.edu.sovereign.core';

export const getTelemetryEndpoint = (): string => {
  const url = Config.EXPO_PUBLIC_TELEMETRY_ENDPOINT;
  if (url) return url;

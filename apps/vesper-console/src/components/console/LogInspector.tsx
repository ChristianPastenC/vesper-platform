        ...
        <div className="space-y-8">
          <div>
            <h3 className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-3 flex items-center gap-2">
              <span className="w-1 h-1 rounded-full bg-blue-400"></span> {t("console.metadata")}
            </h3>
            <div className="bg-black/30 rounded-xl border border-white/5 p-4 text-sm space-y-3 font-mono shadow-inner">
              <div className="flex justify-between items-center"><span className="text-slate-500">{t("console.event_id")}</span> <span className="text-slate-300 truncate w-32 bg-white/5 px-2 py-0.5 rounded text-right" title={selectedLog.id}>{selectedLog.id.split('-')[0]}</span></div>
              <div className="flex justify-between items-center"><span className="text-slate-500">{t("console.timestamp")}</span> <span className="text-slate-300 bg-white/5 px-2 py-0.5 rounded">{timeStr}</span></div>
              <div className="flex justify-between items-center"><span className="text-slate-500">{t("console.event_code")}</span> <span className="text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">0x00{selectedLog.metric_type}</span></div>
              <div className="flex justify-between items-center"><span className="text-slate-500">{t("console.component")}</span> <span className="bg-white/5 px-2 py-0.5 rounded text-slate-300">{details.source}</span></div>
              <div className="flex justify-between border-t border-white/10 pt-3 mt-3 items-center">
                <span className="text-slate-400 font-semibold">{t("console.raw_value")}</span>

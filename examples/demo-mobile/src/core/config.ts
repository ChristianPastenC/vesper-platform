...
export const getApiUrl = (): string => {
  ...
  if (__DEV__) {
    ...
    console.warn(`[Config] API_URL is not set. Falling back to ${fallback}`);
    return fallback;
  }
  throw new Error('[Config] API_URL is required in production.');
};

export const getTestCardNumber = () => Config.TEST_CARD_NUMBER ?? '4242424242424242';

/**
 * Telemetry ingestion is opt-in: only enabled once a developer generates an
 * API Key in the web-support-portal and drops it into TELEMETRY_API_KEY.
 */
export const getTelemetryApiKey = (): string | undefined =>
  Config.TELEMETRY_API_KEY || undefined;
...
export const getTelemetryBundleId = (): string =>
  Config.TELEMETRY_BUNDLE_ID ?? 'mx.edu.sovereign.core';

export const getTelemetryEndpoint = (): string => {
  const url = Config.TELEMETRY_ENDPOINT;
  if (url) return url;

...
export const encodeJsonBody = (value: unknown): Uint8Array => {
  return TEXT_ENCODER.encode(JSON.stringify(value));
};

export const encodeTextBody = (text: string): Uint8Array => {
...
export const appendHeaderToBinary = (
...
export const decodeHeaders = (buffer: Uint8Array): Record<string, string> => {
...
export const decodeBody = (buffer: Uint8Array): string => {
 ...
 */
export const serializeAdapterRequest = (request: {
  method: string;
  url: string;
  encodedHeaders?: Uint8Array;
  ...
  const body: Uint8Array =
    request.body !== undefined && request.body !== null ? request.body : new Uint8Array(0);

  const timeoutMs = request.timeoutMs ?? 0;

  const totalLength = 4 + method.length + 4 + url.length + 4 + hdrs.length + 4 + body.length + 4;
...
export const deserializeAdapterRequest = (
  buffer: Uint8Array,
): Pick<
  SovereignAdapterRequest,
  'method' | 'url' | 'encodedHeaders' | 'body' | 'timeoutMs'
> | null => {
